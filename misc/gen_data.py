...
while not start:
    ...
    cv2.putText(test_img,
                ...
                0.4, (255, 0, 0), 1)
    cv2.putText(test_img,
                f"Ejemplos:{available_samples}/{total_samples}",
                (x, y + h + 30),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.4, (255, 0, 0), 1)

    resized_img = cv2.resize(test_img, (1000, 700))
    cv2.imshow('Analisis', resized_img)
...
for i in range(available_samples, total_samples):
        ...
        cv2.rectangle(test_img, (x, y), (x + w, y + h), (0, 0, 255), 2)
        cv2.putText(test_img,
                    f"Capturando {current_class}", (x, y-5),
